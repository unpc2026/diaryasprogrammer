# PHP Behind The Scene
  ...
  ## PHP Code Execution via Web Server (keywords : how php works with the web server)
  ...
  - The typical flow can be visualized in the following steps:
    ...
    7. Browser Renders Page: The user's web browser receives the HTML response and renders the complete, dynamic web page on the user's screen. The user never sees the underlying PHP source code, only the resulting HTML. 
